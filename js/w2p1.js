let Weight = (function () {
	...
	Constructor.prototype.convertTo = function (unit) {
		...
		this.weight = convertWeight(this.weight, this.settings.unit, unit);
		this.settings.unit = unit;
		return this;
	}

	/**
	 * Change abbreviation setting
	 * @param {boolean} abbr Whether to abbreviated or give full name of unit. Must be true, false, 0 or 1
	 * @see units object for abbreviated and full names
	 */
	Constructor.prototype.changeAbbr = function (abbr) {
		let bool = [1, 0, true, false];
		if (!bool.includes(abbr)) throw 'Not a valid boolean value.';
		this.settings.abbr = abbr;
		return this;
	}
	...
	Constructor.prototype.addWeight = function (weight, unit) {
		...
		if (isNaN(weight)) throw 'Not a valid number.';
		if (!units.hasOwnProperty(unit)) throw 'Not a valid unit.';
		let addedWeight = addWeight(weight, unit, this.weight, this.settings.unit);
	...
	Constructor.prototype.subtractWeight = function (weight, unit) {
		...
		if (isNaN(weight)) throw 'Not a valid number.';
		if (!units.hasOwnProperty(unit)) throw 'Not a valid unit.';
		let subtractedWeight = subtractWeight(weight, unit, this.weight, this.settings.unit);
	...
	for (let key in units) {
		...
		Constructor.prototype[`to_${key}`] = function () {
			this.weight = convertWeight(this.weight, this.settings.unit, key);
			this.settings.unit = key;
			return this;
		}
		...
		Constructor.prototype[`add_${key}`] = function (weight) {
			weight = parseFloat(weight);
			if (isNaN(weight)) throw 'Not a valid number.';
			addedWeight = addWeight(weight, key, this.weight, this.settings.unit);
			if (!addedWeight) return;
			this.weight = addedWeight;
			return this;
		}
		...
		Constructor.prototype[`subtract_${key}`] = function (weight) {
			weight = parseFloat(weight);
			if (isNaN(weight)) throw 'Not a valid number.';
			subtractedWeight = subtractWeight(weight, key, this.weight, this.settings.unit);
			if (!subtractedWeight) return;
			this.weight = subtractedWeight;
			return this;
		}
	...
	Constructor.prototype.format = function () {
		//abbr = typeof abbr === 'boolean' ? abbr : this.settings.abbr;
		let unit = this.settings.unit;
		if (!this.settings.abbr) {
			unit = units[this.settings.unit];
			if (this.weight !== 1) unit += 's';
		}
...
let form = document.getElementById('form'),
	...
	abbrChecked = 1,
	formWeight = new Weight(0, {unit: 'g', abbr: 1});
...
// CONSOLE TESTS
let w1 = new Weight(32, {unit: 'lb', abbr: 0});
w1.add_lb(10).subtract_g(3).to_g();
console.log('Test: started with 32 lb, added 10 lb, subtracted 3 g, converted to grams, formatted with abbr set to 1: ' + w1.format());
